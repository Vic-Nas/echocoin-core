...
# ---------------------------------------------------------------------------
# 7. Censorship resistance (whitepaper Section 4)
# ---------------------------------------------------------------------------

class TestCensorshipResistance:
    def test_censorship_tracker_scores_missing_tx_down(self):
        """A tx excluded from multiple non-full blocks should lower acceptance probability."""
        from node import CensorshipTracker
        tracker = CensorshipTracker()
        pending = frozenset(["tx_a", "tx_b"])

        # Block 1 -- confirms tx_a, tx_b missing, block not full
        tracker.update(confirmed={"tx_a"}, pending=pending, block_is_full=False)
        # tx_b now has age=1 (score=1.0 per whitepaper: first miss is timing noise)

        # Block 2 -- tx_b still missing, block not full
        tracker.update(confirmed=set(), pending=pending, block_is_full=False)
        # tx_b has age=2 -> score=0.5

        score = tracker.score(confirmed=set(), pending=pending)
        assert score <= 0.5  # degraded

    def test_censorship_tracker_full_block_does_not_age_tx(self):
        """Whitepaper Section 4: effective_age only increments on non-full blocks."""
        from node import CensorshipTracker
        tracker = CensorshipTracker()
        pending = frozenset(["tx_a"])

        # Full block -- tx_a excluded but block is full
        tracker.update(confirmed=set(), pending=pending, block_is_full=True)
        # No aging should happen
        score = tracker.score(confirmed=set(), pending=pending)
        assert score == 1.0  # tx_a age is 0 -> score stays 1.0

    def test_censorship_tracker_evicts_confirmed_tx(self):
        from node import CensorshipTracker
        tracker = CensorshipTracker()
        pending = frozenset(["tx_a"])
        tracker.update(confirmed=set(), pending=pending, block_is_full=False)
        # Now tx_a is confirmed and no longer in pending
        tracker.update(confirmed={"tx_a"}, pending=frozenset(), block_is_full=False)
        # _age should not hold stale entries
        score = tracker.score(confirmed=set(), pending=frozenset())
        assert score == 1.0
