...
**Proof-of-work's reward scales with hash rate, with no ceiling.** Doubling your hash rate doubles your expected share of every block, forever. That unbounded incentive is what drove Bitcoin mining from CPUs to GPUs to ASICs, a hardware gap that reached 10,000x or more over commodity hardware, and an enormous, ever-growing energy bill along with it.

**A VDF has a hard floor instead of an open ceiling.** Each block's challenge is a single sequential computation, about 120 seconds here, that more hardware cannot shrink below. Racing to build a block is not a matter of buying more machines and running more attempts in parallel; it is a matter of how fast one chain of sequential steps can be evaluated, and that has a hard floor set by the arithmetic itself.

**That floor still leaves room for specialized hardware, just a small, bounded amount.** Chia Network's 2019 public hardware competition for the same class-group-based VDF construction used here found real, specialized implementations beating commodity software by something like 3 to 10 times, not nothing. But that gap is far narrower than proof-of-work's, and more importantly it does not grow the same way: there is no unbounded reward for adding more of that hardware in parallel, since a single VDF evaluation cannot be split across machines. The design avoids proof-of-work's resource-consumption spiral structurally, from the shape of the problem, rather than by adding a separate "green" rule on top.

**Within that hardware band, it is a race, not a proportional lottery.** A core outside the top few-times band does not win a proportionally smaller share the way lower hash rate does under proof-of-work; it simply loses whenever a faster builder is competing. So the practical cost of a competitive identity is not just any core, it is a core within that top band. That band still moves as general hardware gets faster, generation to generation, the same as it does for any workload; nobody escapes buying newer hardware over time. What is bounded is how much each generation can widen the gap: the arithmetic floor caps how much further specialization can go, unlike proof-of-work's ASICs, which kept opening a wider lead with each new generation instead of converging toward one.

**What that cost does not do, at any generation, is cap how many identities one operator runs.** Nothing in the protocol limits the number of independent builder addresses a single operator holds, so total participation, and the energy that comes with it, still scales with how much an operator is willing to spend, the same way Bitcoin's hash rate does. A VDF narrows the hardware advantage within one identity; it does not, and cannot by itself, limit the number of identities.

**The mechanism that makes more identities exactly proportional, not better than proportional, is the tie-break rule.** Within the top hardware band, competing builders finish close enough in real time that ties are the normal case, not an edge case, and a tie is broken on whichever builder's VDF output is numerically lowest. That output is a deterministic function of the previous block's hash and the builder's own address, but across different addresses it is cryptographically indistinguishable from a random draw. Each address that completes a genuine, full-length VDF evaluation for a given height gets exactly one such draw, and the lowest one wins.

**That draw is also where Sybil resistance is actually priced.** Unlike proof-of-work's nonce search, there is no way to grind for a better draw: a VDF cannot be sped up, retried, or partially computed for credit, so the only way to hold more than one ticket is to run more than one complete, real evaluation, on separate hardware, under a separate address. Splitting one machine into many addresses does not get more tickets this way, it gets fragments too slow to finish before the draw closes; buying ten complete machines instead of one gets exactly ten honest tickets, at ten times the real cost. Sybil resistance here is priced in real machine-time per ticket, not a coin-denominated registration fee, and it is why the design does not need proof-of-work's ever-escalating optimization race to stay Sybil-resistant: the cost scales linearly with participation rather than compounding the way a hash-rate arms race does.

**This was checked, not assumed.** Both claims, that running many addresses at the same hardware tier does exactly proportionally well and that splitting one machine's throughput across several addresses does worse, were checked against a Monte Carlo model of the mechanism above, not left as intuition. A coin-denominated fee to register a builder address was considered as an additional deterrent and rejected: it changes neither result, since the economics here only ever track hardware tier, not balance, and it directly conflicts with "no premine, no authority, every node earns", a fee payable only from an existing balance closes the door on a new node holding nothing yet, which is exactly the node this design means to let in.

**One Sybil surface sits outside this reward-layer analysis entirely: eclipse attacks on peer discovery**, a node's local view of the network being crowded out by attacker-controlled addresses. That is handled separately, by capping how many peers from the same address subnet a node will admit into its own peer table, not by anything priced in coin.
