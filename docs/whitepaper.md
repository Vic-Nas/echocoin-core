...
That gap also changes the shape of competition, not just its size. A VDF round is a race to finish first, not a per-attempt lottery: a core outside that top few-times band does not win a proportionally smaller share the way lower hash rate does under proof-of-work, it simply loses whenever a faster builder is competing. So the practical cost of a competitive identity is not just any core, it is a core within that top few-times band. That band still moves as general hardware gets faster, generation to generation, the same as it does for any workload; nobody escapes buying newer hardware over time. What is bounded is how much each generation can widen the gap, since the arithmetic floor caps how much further specialization on top of ordinary hardware can go, unlike proof-of-work's ASICs, which kept opening a wider lead over commodity hardware with each new generation rather than converging toward one. What that cost does not do, at any generation, is cap how many such cores one operator runs. Nothing in the protocol limits the number of independent builder addresses a single operator holds, so total participation, and the energy that comes with it, still scales with how much an operator is willing to spend, the same way Bitcoin's hash rate does. A VDF narrows the hardware advantage within one identity; it does not, and cannot by itself, limit the number of identities.

The mechanism behind that proportionality is worth spelling out, since it is not an incidental side effect, it is what the tie-break rule is actually for. Within the top hardware band, competing builders finish close enough in real time that ties are the normal case, not an edge case, and a tie is broken on whichever builder's VDF output is numerically lowest. That output is a deterministic function of the previous block's hash and the builder's own address, but across different addresses it is cryptographically indistinguishable from a random draw. So each address that completes a genuine, full-length VDF evaluation for a given height gets exactly one such draw, and the lowest one wins. Unlike proof-of-work's nonce search, there is no way to grind for a better draw: a VDF cannot be sped up, retried, or partially computed for credit, so the only way to hold more than one ticket in that draw is to run more than one complete, real evaluation, on separate hardware, under a separate address. That is precisely the resource Sybil resistance here is priced in: not a registration fee paid in the coin itself, but a full VDF's worth of real machine-time per ticket. An attacker splitting one machine into many addresses does not get more tickets this way, it gets fragments too slow to finish before the draw closes; buying ten complete machines instead of one gets exactly ten honest tickets, at ten times the real cost. The proportional outcome the rest of this section describes is that draw, working as intended, and it is also why the design does not need proof-of-work's ever-escalating optimization race to stay Sybil-resistant: the cost that matters is denominated in complete machines run in real time, a cost that scales linearly with participation rather than compounding the way a hash-rate arms race does.

That last point is worth stating precisely rather than leaving as an assumption. Running many independent builder addresses at the same hardware tier does no better than proportional: if every address draws its own outcome from the same real-time race, its chance of winning is exactly its share of addresses at that tier, the same symmetry that already governs a single race. Splitting one machine's throughput across several addresses instead of running it as one does worse than proportional, since each fragment then runs the full 120 seconds at a fraction of the speed and stops being competitive past a small number of fragments. Neither of these was assumed; both were checked against a Monte Carlo model of the mechanism above. A coin-denominated fee to register a builder address was considered as an additional deterrent and rejected, since it changes neither result, the economics here only ever track hardware tier, not balance, and it directly conflicts with "no premine, no authority, every node earns": a fee payable only from an existing balance closes the door on a new node holding nothing yet, which is exactly the node this design means to let in. The one Sybil surface this does not address at the reward layer is eclipse-style attacks on peer discovery, a node's local view of the network being crowded out by attacker-controlled addresses. That is handled separately, by capping how many peers from the same address subnet a node will admit into its own peer table, not by anything priced in coin.
